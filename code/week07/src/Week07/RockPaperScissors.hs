...
transition game s r = case (stateValue s, stateData s, r) of
    ...
    (v, GameDatum _ (Just c2), Reveal _ c1)
        | lovelaces v == (2 * gStake game) && c2 /= c1   -> Just ( Constraints.mustBeSignedBy (gFirst game)                     <>
                                                       Constraints.mustValidateIn (to $ gRevealDeadline game)
                                                     , State Finished mempty
                                                     )
    (v, GameDatum bs (Just c2), Reveal _ c1)
        | lovelaces v == (2 * gStake game) && c2 == c1   -> Just (
                                                       Constraints.mustValidateIn (to $ gRevealDeadline game)
...
{-# INLINABLE check #-}
-- It will be called on every state transition.
-- It will return true in all cases except it matches the case that contains the main logic here:
-- where the first player has moved (`(GameDatum bs (Just c))`) and where the redeemer is `Reveal` (`(Reveal nonce firstGC)`)
check :: BuiltinByteString -> BuiltinByteString -> BuiltinByteString -> GameDatum -> GameRedeemer -> ScriptContext -> Bool
check bsRock' bsPaper' bsScissors' (GameDatum bs (Just c)) (Reveal nonce firstGC) _ = (bsMatches nonce firstGC bs) && (c == firstGC || (beats firstGC c))
    where
      bsMatches :: BuiltinByteString -> GameChoice -> BuiltinByteString -> Bool
      bsMatches n firstGameChoice firstHashedChoice =
        sha2_256 (n `appendByteString` case firstGameChoice of
          Rock     -> bsRock'
          Paper    -> bsPaper'
          Scissors -> bsScissors') == firstHashedChoice
...
--        bs     = sha2_256 $ fpNonce fp `appendByteString` if c == Zero then bsZero else bsOne
        bs     = sha2_256 $ fpNonce fp `appendByteString` case c of
          Rock     -> bsRock
          Paper    -> bsPaper
          Scissors -> bsScissors
    void $ mapError' $ runInitialise client (GameDatum bs Nothing) v
    logInfo @String $ "made first move: " ++ show (fpChoice fp)
    logInfo @String $ "first pubKeyHash: " ++ show pkh
    tell $ Last $ Just tt
...
secondGame sp = do
    ...
    let game   = Game
        ...
        client = gameClient game
    -- m - the UTxO representing the game
    m <- mapError' $ getOnChainState client
    case m of
        ...
        Just (o, _) -> case tyTxOutData $ ocsTxOut o of
            GameDatum _ Nothing -> do
                ...
                m' <- mapError' $ getOnChainState client
                case m' of
                    -- Nothing - because the first player already claimed, ended the game and emptied the state I guess?
